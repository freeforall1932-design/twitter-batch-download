...
| GIF → real .gif + quality guarantees | **Done (v3.6), quality modes v3.14, WebP + fallback v3.15, needs live spot-check** | `normalizePhotoUrl` forces `name=orig` on every source; videos keep highest-bitrate MP4; GIFs convert MP4→GIF89a (balanced default or `gif-max`: per-frame local palettes + Floyd–Steinberg, 25 fps/≤1920 px), → animated WebP (`webp`, true color via browser encoder + `lib/webpEncoder.js`) or → true-color APNG (`apng`, full RGBA via `lib/apngEncoder.js`) in the offscreen document, chunked back to the worker; on any failure the other animated formats are retried in fidelity order and MP4 is the last resort; `gifOutput` toggle (5 options, default balanced GIF). |
