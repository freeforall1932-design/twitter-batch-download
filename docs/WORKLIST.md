...
| GIF → real .gif + quality guarantees | **Done (v3.6), quality modes v3.14, needs live spot-check** | `normalizePhotoUrl` forces `name=orig` on every source; videos keep highest-bitrate MP4; GIFs convert MP4→GIF89a (balanced default or `gif-max`: per-frame local palettes + Floyd–Steinberg, 25 fps/≤1920 px) or → true-color APNG (`apng`, full RGBA via `lib/apngEncoder.js`) in the offscreen document, chunked back to the worker, with MP4 fallback on any failure; `gifOutput` toggle (4 options). |
