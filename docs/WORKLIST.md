...
- **DONE — audit this repository for `onDeterminingFilename`.** No registration exists in `extension/`, `firefox-extension/`, or `source/archive-enabled/`; no lazy/permanent listener fix is required here.
- **DONE — map download and naming paths.** The workers call `chrome.downloads.download()` directly with filenames produced by the existing naming/path engine. `chrome.downloads.onChanged` is completion/progress bookkeeping only.
- **DONE — regression protection.** `tests/background.test.js` now asserts that the worker variants do not register a global filename-determination listener.
- **OPEN — live verification.** With another naming downloader installed, download through that other extension outside X/Twitter and confirm Chrome does not blame X Media Downloader. Separately verify this product's existing `{user} - {text} - {id}` / folder behavior on X.
- **POLICY — do not add `onDeterminingFilename` merely to improve names.** Direct `chrome.downloads.download({ filename })` already preserves this product's naming behavior without joining the global naming chain. If a future feature truly requires a guard, it must be lazy, own-download scoped, never suggest an empty filename, and have listener-lifetime tests.
...
_Last audited: 2026-09-03 (this session = **v3.15 animated WebP mode + animated-format fallback chain**: "Animated posts save as" gained an animated **WebP** choice (new dependency-free `lib/webpEncoder.js` wraps browser-encoded static WebP frames into the VP8X/ANIM/ANMF container, true color, small files); conversion failures now fall back automatically through `ANIMATED_OUTPUT_FIDELITY` (`apng → webp → gif-max`) with the MP4 kept only as the last resort — and on review the balanced global-palette GIF was removed entirely (one GIF mode only, `gif-max` now the default; legacy `gif`/unknown values map to `gif-max`) because WebP covers the small/simple niche with true color while a single-table GIF bands on color shifts — "use GIF before MP4"; `convertFallbackChain` + `normalizeGifOutput` updated in both workers, sidepanel trio re-synced; new `webp-encoder.test.js` (7) + fallback/webp media-kinds tests; reference-decoder review confirms APNG/WebP/GIF are all genuine animated formats. Manifest **3.14.0 → 3.15.0**; full offline suite **194 / 0 fail**; no release zip cut yet (pending your review). Previous audit 2026-09-03: **v3.14 output quality modes — max-quality GIF + true-color APNG**: "GIF posts save as" now offers balanced GIF (default, unchanged), `gif-max` (25 fps / ≤1920 px / per-frame local 256-color palettes + Floyd–Steinberg dithering / ≤256 MB), `apng` (true-color RGBA frames via new dependency-free `lib/apngEncoder.js`, same caps) and MP4; offscreen conversion is chunked (two-message `offscreenConvertGif` → `offscreenConvertGifChunk` relay, 3 MB binaries, 15-min TTL, legacy one-shot path kept); both workers pass the mode through, keep data:-URL saves (so `.apng` lands in the master folder), and degrade every failure to MP4; Side Panel + normalize updated in both ports; new tests: APNG suite (6, CRC/chunk/sequence/byte-exact) + 4 GIF max-quality regressions + chunked-relay/modernize media-kinds tests — a local-palette header-per-frame encoder bug and an APNG acTL patch/CRC bug were fixed along the way. Manifest **3.13.0 → 3.14.0**; workflows also cover `firefox-extension/**` + `source/archive-enabled/**`; full offline suite **183 / 0 fail**; no release zip cut yet (pending your review). Previous audit 2026-09-03: **v3.13 leanness pass — dead archive code stripped, GIF conversion restored (GIF-only offscreen)**: per-post ZIP/CBZ/PDF stays retired (preserved under `source/archive-enabled/`, tests pinned there); the ~230-line dead archive section was removed from both shipped workers (`runArchivePass`/`buildArchiveInWorker`/`sendArchiveJobToOffscreen`/`archiveGroups`/`effectiveGroupFormat`/`archiveEntryExtension`/`archivedKinds`/`buildRunNotices`) and the vestigial archive UI removed (defaultFormat/jobFormat selects, queueNotices box + CSS, archive preview branch, archive settings keys); the Chrome build re-adds a small **GIF-only** offscreen document + `offscreen` permission so "GIF posts save as → Real .gif" actually works again (the Side Panel select is now wired to storage); Firefox keeps the MP4 clip (no `chrome.offscreen`). Manifest **3.12.0 → 3.13.0**; full offline suite **169 / 0 fail** (168 + stale-format regression). No release zip cut yet (pending your review). Previous audit (2026-09-03): **v3.12 archive-output retirement, source split + test retarget**: archive-specific tests were retargeted to `source/archive-enabled/` (new `tests/archive-background.test.js` + archive-lib/pdf-builder/zip-writer) and the full offline suite was **168 / 0 fail** — the 8 broken archive tests are runnable again. Previous audit (2026-09-03): **v3.11 per-user folders + random-naming PENDING REVIEW**: raw downloads default to `Downloads/XMedia/<user>/<post name>/001.jpg` — master folder → one folder **per user** → per-post folder — so a user's media from the home timeline, a profile and its `/media` page all live together, and the folder doubles as visual dedupe on top of v3.10's byte + source-URL verification; `userFolders` toggle (default ON) restores the old `XMedia/<post name>/…` layout; archives (ZIP/CBZ/PDF) cannot create folders, so the username is **forced into the file name** whenever the template would omit it (`{id}` → `nasa - 111.cbz`); legacy queue rows without naming metadata still get their user folder from `item.author`. **The garbled-random-name problem is on this work list as PENDING REVIEW — NOT fixed until the user tests and confirms; root cause still unconfirmed.** Manifest 3.10.0 → 3.11.0, **167 offline tests** (+3), `firefox-extension/` re-synced, no release zip cut yet (pending user review). Previous-but-one audit (2026-09-03): **v3.10 byte-identical + source-URL duplicate verification**: new `lib/dedupe.js` (streaming SHA-256 + canonical source URL); `downloadedMediaRecordsV1` record store (`{id, mediaKey, url, urlKey, hash, size, filename, at}`); `downloadFile` checks URL first (no network), then hashes the bytes and skips same-URL / byte-identical media — the row shows `completed · duplicate` instead of saving a `(1)` copy; completion writes hash + URL + real filename (queue rows via `onChanged`/restart reconcile, direct saves via a pending-record map); canonical URL joins `id`/`mediaKey` as a third queue identity + "Skip already downloaded" record check; archive pass skips already-verified groups and records per-entry + archive digests; `verifyDuplicates` toggle added to Output settings (default ON, best-effort); legacy flat-path sanitizers strip bidi controls so names cannot garble. Manifest 3.9.0 → 3.10.0, 164 offline tests (+14), Firefox port re-synced, release zip re-cut. Prev = **v3.9 virtualization-proof capture**: X removes articles that scroll off-screen and capture was scan-only, so posts inserted and removed between two scans were never listed at all — measured **81 of 207 photos and 39 of 103 posts** on a fast scroll. `harvestMutationArticles()` now reads the observer's `addedNodes` **and** `removedNodes`, lifting that to 204/101 (fast) and 207/103 (normal) with **0 duplicates**; videos went **1 → 17** because a harvested video post is now queued for its per-post resolve instead of vanishing. Dedupe itself was already airtight and is now test-pinned. Prev = **v3.8 Rescan restores deleted rows**: `Rescan tab` and `Fetch media` now forget the tab's "already listed" memory and re-list the posts on screen, so rows the user deleted come back; new **Remove selected** button; `skippedDownloaded` reporting so "nothing came back" always names a reason. Prev = **v3.7 Fetch button**: shallow auto-fetch on tab open/route change, in-page Fetch dock, hybrid deep fetch (scroll → silent GraphQL fill), Side Panel Fetch/Rescan/Reload-tab controls, plus an 8-item missing-logic audit — see the newest IMPROVEMENT_LOG entry. Prev = Firefox port: separate firefox-extension/ folder MV2 sidebar_action + compat shims, README, manifest variant. Prev = v3.6.3 naming-degarble + perf queue:
...
| Per-post ZIP/CBZ/PDF output | **Retired (v3.12) — code stripped (v3.13), tests pinned** | Queue output is separate original-resolution files only; no archive code remains loaded or reachable in the shipped UI/worker (the dead archive section was removed from both workers in v3.13). The pre-v3.12 implementation is preserved under `source/archive-enabled/` and its historical suites stay runnable (retargeted to that source variant: `tests/archive-background.test.js` + archive-lib/pdf-builder/zip-writer). | One archive per post (≤4 items), assembled in the offscreen document, saved via `<a download>` anchor (blob-filename quirk); worker data-URL fallback. v3.6: PDF is photos-only (GIF/video posts degrade PDF→ZIP); GIFs archive by default, videos opt-in; warnings at queueStart. NOT the removed whole-batch ZIP. |
| GIF → real .gif + quality guarantees | **Done (v3.6), quality modes v3.14, WebP + fallback v3.15, needs live spot-check** | `normalizePhotoUrl` forces `name=orig` on every source; videos keep highest-bitrate MP4; GIFs convert MP4→GIF89a (`gif-max` only since the v3.15 review: per-frame local palettes + Floyd–Steinberg, 25 fps/≤1920 px; the retired balanced encoder remains only inside `lib/gifEncoder.js`), → animated WebP (`webp`, true color via browser encoder + `lib/webpEncoder.js`) or → true-color APNG (`apng`, full RGBA via `lib/apngEncoder.js`) in the offscreen document, chunked back to the worker; on any failure the other animated formats are retried in fidelity order and MP4 is the last resort; `gifOutput` toggle (4 options, default `gif-max`). |
