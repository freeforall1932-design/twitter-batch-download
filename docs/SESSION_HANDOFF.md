...
extension/offscreen.html   # + offscreen.js: GIF-only MP4→.gif conversion
                           # (chrome.runtime ONLY — re-added in v3.13 for the
                           # restored raw-GIF path; no archive code)
...
## 4. Current architecture (v3.13.0)
...
| `offscreen.html/js` | **Shipped (v3.13, GIF-only):** converts X's MP4 "GIF" clips through `<video>`+canvas (12 fps, ≤30 s/≤360 frames/≤720 px/≤40 MB) into real .gif bytes and returns them to the worker as base64 (data: URLs keep the master-folder subpath). Handles ONLY `offscreenConvertGif`; exposes ONLY `chrome.runtime`. **Preserved variant (archive-enabled):** also assembled ZIP/CBZ/PDF blobs and saved them via an in-document `<a download>` anchor — kept under `source/archive-enabled/`, not shipped. |
...
**Worker → offscreen (v3.13, shipped GIF-only document):**
`offscreenConvertGif` `{ job: { url } }` → `{ ok, base64 }` /
`{ ok: false, error }`, answered via `sendResponse`. The shipped
`offscreen.js` handles ONLY this message; `offscreenBuildArchive` exists
only in the preserved `source/archive-enabled/` variant and is pinned by
`tests/archive-background.test.js`. Chrome: GIF conversion runs (no
...
`gif-encoder.test.js` round-trips encoded GIFs
