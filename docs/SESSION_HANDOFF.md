...
`deepFetchRemote`, `showFetchButton`, `dockHidden`, `scans`), `scrollRescan`
(shallow pass only — now sent by the panel's **Rescan tab**).
...
node --test tests/*.test.js            # 135 tests (+10 in v3.7)
...
`safeSend` before it was kept). The shim grew three backwards-compatible
...
start on a profile URL. `tests/downloader.test.js` runs the real `background.js` (plus the
...
1. **Live spot-check v3.7's Fetch button first** — WORKLIST P0 item 16 (the
...
2. **Then live spot-check v3.4 + v3.5 + v3.6 + v3.6.1** — WORKLIST P0 items 12 and 14.
...
3. **Cut the first release zip** — `scripts/package-release.sh` →
   `releases/x-media-downloader-v3.7.0.zip` is **cut and offline-verified**
   (v3.6.1/v3.6.3 before it were too)
...
4. P1 afterwards: Side Panel diagnostics + sanitized copy-debug-report,
...
5. Settled decisions that stay settled: separate scroll/remote lists (do not
   ...
   decisions 19–23: shallow automatic / deep on click, one in-page dock, the
   silent fill reuses discovery and lands in the Remote list, run tokens on
   every loop, incremental replays.
6. Before finishing: update all three docs (log entry, worklist statuses,
   this file).
