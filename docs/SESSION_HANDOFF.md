...
**Prepared:** 2026-09-01 · **Extension version:** 3.5 · **Status:** v3.5 = v3.4 + the media output upgrade ported from nh-dw-2.0 (master folder for raw downloads, per-post ZIP/CBZ/PDF via an offscreen document, naming-scheme checkboxes, offline CI workflow). All 88 offline tests green. Still pending live-X: the v3.4 quote-card spot-check AND the v3.5 output spot-check (WORKLIST P0 items 12 + 14).
...
_Latest input (2026-09-01, session brief):_ port three proven features from
...
| `offscreen.html/js` | Archive assembly: fetch post photos, build ZIP/CBZ/PDF blob, save via in-document `<a download>` anchor. Exposes ONLY `chrome.runtime` — settings arrive relayed in the job message. |
...
| `sidepanel.html/js/css` | Two-tab Side Panel: Scroll capture + Remote fetch. One download action, live active-tab status pill, per-row remove, skip-already-downloaded toggle, **Include quoted** switches, `Clear finished` / `Reset downloaded history` buttons. v3.5: **Output settings** card (master folder, default format, name-template checkboxes + live preview + custom-template input — the ONLY writer of the sync output settings) and the dock's per-job **Save photo posts as** picker. |
| `popup.html/js` | Side Panel launcher + capture status line. No scroll/download loop. |
| `tests/` | `background.test.js`, `content.test.js`, plus v3.5: `naming.test.js`, `zip-writer.test.js`, `pdf-builder.test.js` (verbatim port), `downloader.test.js` (real worker in a VM: master-folder + archive pipelines), `helpers/load-background.js`. |
...
`mediaIndex` / `isQuote`), `downloadFile` (v3.5: optional `item` with the
owning post's metadata so the path honors master folder + template; without
it the legacy `filename` is used verbatim)

**Worker → offscreen:** `offscreenBuildArchive`
`{ job: { format, filename, images: [{ url, name }] } }`, answered via
`sendResponse` `{ ok }` / `{ ok: false, error }`. Not part of the UI contract
test (no UI surface sends it); the offscreen document handles it in
`offscreen.js`.
...
**chrome.storage.sync (v3.5, written ONLY by the Side Panel Output settings
card):** `rawMasterFolder` (default `"XMedia"`; **empty string = OFF**),
`nameTemplate` (default `"{user} - {text} - {id}"`), `outputFormat`
(default `"raw"`, whitelist raw|zip|cbz|pdf).
...
- **Media output upgrade (v3.5, checklist item 14)** — never run in a
  ...
  names, dock picker not touching the stored default.
...
node --check extension/background.js extension/content.js extension/popup.js \
             extension/sidepanel.js extension/injected.js extension/offscreen.js \
             extension/lib/naming.js extension/lib/zipWriter.js extension/lib/pdfBuilder.js
node --test tests/*.test.js            # 88 tests
...
and the offscreen job relay. **Extend these rather than testing by hand** —
every future capture or output bug should land as a failing test first.

CI (`docs/ci/extension-tests.yml`, to be installed by hand as
`.github/workflows/extension-tests.yml` — the Arena GitHub App lacks the
`workflows` scope; see `docs/ci/README.md`) runs exactly the offline set
above plus a packaging smoke. It deliberately has NO real-browser job (§7).

---

## 9. Next session priorities

1. **Live spot-check v3.4 + v3.5** — WORKLIST P0 items 12 and 14. Reload
   `extension/` unpacked (manifest 3.5, "ask where to save" OFF), then:
   the quote-card case (item 12), a 4-photo post in raw mode
   ...
   names, and the dock picker not persisting. If an archive still saves
   under a UUID, capture the Chrome version — that is the blob-filename bug
   the anchor mechanism is supposed to dodge.
2. **Cut the first release zip** (`scripts/package-release.sh` →
   `releases/x-media-downloader-v3.5.zip`) and confirm it loads from the
...
3. P1 afterwards: Side Panel diagnostics + sanitized copy-debug-report,
   explicit Include replies switch (quoted shipped in v3.4), video quality
   preference (name templates shipped in v3.5).
