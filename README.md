...
- **Separate original-resolution files only (v3.12, GIF conversion restored v3.13, quality modes v3.14)** — The queue downloads each selected media item independently: original-resolution photos (`name=orig`), highest-bitrate MP4 videos, and GIFs as a real `.gif` or true-color APNG per the output setting (converted from X's silent MP4 clips in a small offscreen document — Chrome; the Firefox port keeps the MP4 clip). The former per-post **ZIP / CBZ / PDF** output (≤4 items of one post, media-kind rules, archive toggles and warnings) was retired from the shipped build and is preserved for reference — **not** a Load-unpacked target — under [`source/archive-enabled/`](source/archive-enabled/README.md). The old multi-GB whole-batch ZIP stays removed.
...
- **GIF posts save as** (v3.6, working again in v3.13, quality modes v3.14) — four choices: real `.gif` — **balanced** (default: 12 fps, ≤720 px, global palette, compact), real `.gif` — **maximum quality** (25 fps, ≤1920 px, per-frame 256-color palettes + Floyd–Steinberg dithering, larger), **APNG — true color** (25 fps, ≤1920 px, no palette, every frame keeps full RGBA; the closest an image format gets to the MP4), or the **original MP4 clips**. Quality modes deliberately trade size for fidelity. Firefox port: conversion is unavailable (no `chrome.offscreen`), so the MP4 clip is kept.
...
7. **GIF/APNG conversion (v3.6, re-added v3.13, quality modes v3.14)** — X serves `animated_gif` media as a small silent MP4 clip. The worker asks a small offscreen document (chrome.runtime only) to convert it per the `gifOutput` setting: balanced GIF89a (`lib/gifEncoder.js`, median-cut palette + LZW, ≤30 s / ≤720 px / ≤40 MB), maximum-quality GIF89a (per-frame local palettes + Floyd–Steinberg dithering, ≤30 s / ≤1920 px / ≤256 MB), or true-color APNG (`lib/apngEncoder.js`, full RGBA frames, same caps). Large results travel back **chunked** (3 MB binary per runtime message); all sizes save via a `data:` URL so they still land inside the master folder with the right `.gif`/`.apng` extension. No offscreen API or conversion failure → the original MP4 is kept, never a failed item. Firefox port: no offscreen → MP4 kept.
...
| `offscreen` | Convert X's silent MP4 "GIF" clips into real `.gif` files (balanced or maximum quality) or true-color APNG (MV3 workers have no `<video>`/canvas). Conversion only — no archive assembly |
...
│   ├── offscreen.html/js      #   MP4 clip → .gif / APNG conversion document (chrome.runtime ONLY)
│   ├── lib/                   #   naming.js, dedupe.js, gifEncoder.js, apngEncoder.js (shared with tests)
